...
本文档详细说明 ReviewYourMind 的技术架构、核心模块和设计决策。

## 整体架构

ReviewYourMind 采用 **Tauri 混合架构**，结合 Web 技术和原生性能优势。
...
- 功能: 加载、保存、验证配置
- 存储: TOML 格式
- 位置: `~/.config/reviewyourmind/config.toml`
...
```bash
# 实时查看日志
tail -f ~/.local/share/reviewyourmind/logs/app.log
```
