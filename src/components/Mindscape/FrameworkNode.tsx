...
function FrameworkNodeComponent({ data, selected }: NodeProps<Node<MindscapeNodeData>>) {
  ...
  const onNavigateToCanvas = data?.onNavigateToCanvas;
  ...
  return (
    <>
      <NodeToolbar position={Position.Top} offset={8}>
        <div className="flex gap-1 bg-slate-800 rounded-lg shadow-lg border border-slate-600 p-1">
          <button
            className="text-xs px-2 py-1 rounded hover:bg-slate-700 text-slate-300 transition-colors"
          ...
          <button
            className="text-xs px-2 py-1 rounded hover:bg-slate-700 text-slate-300 transition-colors"
            onClick={(e) => { e.stopPropagation(); onNavigateToCanvas?.(id); }}
          >
            Open Canvas
          </button>
        </div>
      ...
      <div
        className={`w-[280px] rounded-xl p-4 cursor-pointer transition-all duration-200 border ${styleClass} ${
